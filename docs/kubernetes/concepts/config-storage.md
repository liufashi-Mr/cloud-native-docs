...
前置：[网络与流量](/kubernetes/concepts/networking)。下一篇：[身份与安全](/kubernetes/concepts/security)，为 API、Pod runtime 和网络访问建立分层边界。容器可写层、bind mount、Volume 与 tmpfs 的生命周期见 [容器存储](/docker-oci/runtime/storage)。
